...
def deep_sync(token):
    ...
    try:
        ...
        while True:
            ...
            sync_status.info(
                f"⏳ Syncing node data packet {skip}..."
            )
        ...
        if not all_recs:
            return None, true_balance, "History stream empty."
        ...
        if not all([d_col, a_col, node_col, lic_col]):
            raise Exception(
                f"Unexpected API schema: {list(df.columns)}"
            )

        # Timestamp fix (mixed ISO support)
...
if raw_input:
    ...
    st.info(
        "System Standby. Please enter your Bearer Token in the sidebar."
    )
